...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [bookDemoContacts, emailFormContacts] = await Promise.all([
      searchAll(
        apiKey,
        mode === 'all'
      ...
      Promise.resolve([]), // No second call needed — both filter groups in one
    ])

    // Deduplicate by contact ID
    const seen = new Set<string>()
    const contacts: any[] = []
    for (const c of [...bookDemoContacts, ...emailFormContacts]) {
      if (!seen.has(c.id)) {
        seen.add(c.id)
        contacts.push(c)
      }
