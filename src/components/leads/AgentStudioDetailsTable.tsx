...
// Mirrors LeadsDetailsTable, but scoped to only Agent Studio form submissions
// (instead of excluding them) — high/medium/low/unknown priority breakdown.
export function AgentStudioDetailsTable({ contactsByPriority }: Props) {
  const [filter, setFilter] = useState<'all' | 'high' | 'medium' | 'low'>('all')

  const all = useMemo(() => {
    const isStudio = (c: LeadContact) => (c.formTypes && c.formTypes.length ? c.formTypes : [c.formType]).includes('Agent Studio')
    return [
    ...
    <div className="rounded-[20px] border border-[#D4CBC0] bg-white p-6 shadow-[0_4px_20px_rgba(40,20,10,.07)]">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4 pb-4 border-b border-[#F2EDE8]">
        ...
        <div className="text-[12px] font-[600] text-[#7A6A60]">
          {filtered.length} of {all.length} studio leads shown
        </div>
      </div>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={10} className="py-8 text-center text-[#7A6A60]">
                  No Agent Studio leads found for the selected priority filter.
                </td>
              </tr>
            )}
          </tbody>
