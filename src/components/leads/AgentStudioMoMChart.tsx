...
// HubSpot-powered — same pattern as LeadsMoMChart's HubSpotMoMChart, but tracks
// only the Agent Studio form-type count instead of excluding it.
export function AgentStudioMoMChart({ weekStart, title, actualLabel = 'Studio Leads' }: Props) {
    ...
    async function fetchAll() {
      const promises = monthsConfig.map(async (m) => {
        try {
          ...
          return { month: m.label, [actualLabel]: data.by_form_type?.['Agent Studio'] || 0 }
        } catch {
          return { month: m.label, [actualLabel]: 0 }
        }
      })
