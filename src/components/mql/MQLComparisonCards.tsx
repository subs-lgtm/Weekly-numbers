...
function ComparisonCard({
  title, currentLabel, prevLabel, currentValue, prevValue, loading,
}: {
  title: string; currentLabel: string; prevLabel: string;
  currentValue: number; prevValue: number; loading?: boolean;
}) {
        ...
        <div className="flex items-end justify-between gap-4">
          ...
          <div className="flex flex-col items-center gap-1 pb-1">
            ...
            <span className="text-[10px] text-[#7A6A60]">
              {diff > 0 ? '+' : ''}{diff.toLocaleString()} MQLs
            </span>
          </div>
          <div className="text-center flex-1">
...
export function MQLComparisonCards({ sectionKey, weekStart }: Props) {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      const [momCurr, momPr] = await Promise.all([
        fetch(`/api/hubspot/mqls?start=${currentMonthStart}&end=${currentMonthEnd}`).then(r => r.json()).catch(() => ({ total: 0 })),
        fetch(`/api/hubspot/mqls?start=${prevMonthStart}&end=${prevMonthEnd}`).then(r => r.json()).catch(() => ({ total: 0 })),
      ])
      if (!cancelled) {
        setMomCurrent(momCurr.total || 0)
        setMomPrev(momPr.total || 0)
        setLoading(false)
      }
    }
